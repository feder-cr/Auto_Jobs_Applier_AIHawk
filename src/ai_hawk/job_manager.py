...
class AIHawkJobManager:
        ...
        for position, location in searches:
            ...
            try:
                while True:
                    ...
                    if time_left > 0:
                        try:
                            user_input = inputimeout(prompt=f"Sleeping for {time_left} seconds. Press 'y' to skip waiting. Timeout 60 seconds : ", timeout=60).strip().lower()
                    ...
                    if page_sleep % 5 == 0:
                        ...
                        try:
                            user_input = inputimeout(prompt=f"Sleeping for {sleep_time / 60} minutes. Press 'y' to skip waiting. Timeout 60 seconds : ", timeout=60).strip().lower()
                        except TimeoutOccurred:
                            user_input = ""  # No input after timeout
            ...
            if time_left > 0:
                # Redundant code block - consider abstracting this to a function
                try:
                    user_input = inputimeout(prompt=f"Sleeping for {time_left} seconds. Press 'y' to skip waiting. Timeout 60 seconds : ", timeout=60).strip().lower()
                except TimeoutOccurred:
                    user_input = ""  # No input after timeout
            ...
            if page_sleep % 5 == 0:
                ...
                try:
                    user_input = inputimeout(prompt=f"Sleeping for {sleep_time / 60} minutes. Press 'y' to skip waiting: ", timeout=60).strip().lower()
                except TimeoutOccurred:
                    user_input = ""  # No input after timeout
    ...
    def write_to_file(self, job, file_name):
        ...
        if not file_path.exists():
            ...
            with open(file_path, "r+", encoding="utf-8") as f:
                ...
                f.truncate()
                logger.debug(
                    f"Job data appended to existing file: {file_name}")
    ...
    def get_base_search_url(self, parameters):
        ...
        experience_levels = [str(i + 1) for i, (level, v) in enumerate(
            parameters.get("experience_level", {}).items()) if v]
        ...
        job_types = [key[0].upper() for key, value in parameters.get(
            "jobTypes", {}).items() if value]

        if job_types:
            url_parts.append(f"f_JT={",".join(job_types)}")

        date_mapping = {"all_time": "",
    ...
    def extract_job_information_from_tile(self, job_tile):
        ...
        try:
            ...
            job_title = job_tile.find_element(
                By.CLASS_NAME, "job-card-list__title").find_element(By.TAG_NAME, "strong").text
            ...
            logger.warning(
                "Some job information (title, link, or company) is missing.")
    ...
    def is_blacklisted(self, job_title, company, link, job_location):
        ...
        title_blacklisted = any(word in job_title_words for word in map(
            str.lower, self.title_blacklist))
        company_blacklisted = company.strip().lower() in (word.strip().lower()
                                                          for word in self.company_blacklist)
        location_blacklisted = job_location.strip().lower() in (word.strip().lower()
                                                                for word in self.location_blacklist)
    ...
    # Is this function necessary? Is it used more than once?
