...
class AIHawkJobManager:
        ...
        for position, location in searches:
            ...
            if page_sleep % 5 == 0:
                ...
                try:
                    user_input = (
                        inputimeout(
                            prompt=f"Sleeping for {sleep_time / 60} minutes. Press 'y' to skip waiting: ", timeout=60
                        )
                        .strip()
                        .lower()
                    )
    ...
    def job_tile_to_job(self, job_tile) -> Job:
        ...
        try:
            job.title = (
                job_tile.find_element(By.CLASS_NAME, "job-card-list__title").find_element(By.TAG_NAME, "strong").text
            )
        ...
        try:
            ...
            logger.debug(f"Job ID extracted: {job.id} from url:{job.link}") if match else logger.warning(
                f"Job ID not found in link: {job.link}"
            )
        except Exception as e:
